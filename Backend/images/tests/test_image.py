...
class TestImageService:
    @pytest.mark.asyncio
    async def test_get_by_cod_success(self, mock_image_repository):
        # Arrange
        cod_image = uuid4()
        cod_user = uuid4()
        ...
        mock_image_repository.find_by_cod = AsyncMock(
        ...
        mock_image_repository.find_by_cod.assert_called_once_with(cod_image)

    @pytest.mark.asyncio
    async def test_get_not_found(self, mock_image_repository):
        # Arrange
        cod_image = uuid4()
        ...
        with pytest.raises(ImageNotFoundError, match=f"The image with id {cod_image} not found"):
            await service.find_by_cod(cod_image)

    @pytest.mark.asyncio
    async def test_upload_success(self, fake_upload_file, mock_image_repository):
        # Arrange
        cod_image = uuid4()
        cod_user = uuid4()
        ...
        mock_image_repository.upload = AsyncMock(
            return_value=expected_image)
    ...
    async def test_delete_success(self, mock_image_repository):
        ...
        mock_image_repository.delete = AsyncMock(
            return_value=expected_image)
        ...
        # Assert
        assert result == expected_image
        mock_image_repository.delete.assert_called_once_with(cod_image)

    @pytest.mark.asyncio
    async def test_delete_not_found(self, mock_image_repository):
        # Arrange
        cod_image = uuid4()

        mock_image_repository.delete = AsyncMock(return_value=None)

        service = ImageService(mock_image_repository)
        service.find_by_cod = AsyncMock(side_effect=ImageNotFoundError(
    ...
    @pytest.mark.asyncio
    async def test_delete_failed(self, mock_image_repository):
        # Arrange
        cod_image = uuid4()
        cod_user = uuid4()
        ...
        service = ImageService(mock_image_repository)
        service.find_by_cod = AsyncMock(return_value=check_image)
